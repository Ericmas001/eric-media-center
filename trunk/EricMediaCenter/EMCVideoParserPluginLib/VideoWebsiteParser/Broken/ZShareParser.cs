...
namespace EMCVideoParserPluginLib.VideoWebsiteParser.Broken
{
    public class ZShareParser
    {
        ...
        {
            string playerURL = StringUtility.Extract(src, "<iframe src=\"", "\"");
            if (playerURL == null)
            ...
            string movie = "http://www.zshare.net/" + StringUtility.Extract(res, "<param name=\"movie\" value=\"", "?");
            string vars = StringUtility.Extract(res, "<param name=\"flashvars\" value=\"", "\"");
            return new FlashViewer(movie, playerURL, vars);*/

            /* Test 2.0
            string id = StringUtility.Extract(playerURL, "&H=", "&");
